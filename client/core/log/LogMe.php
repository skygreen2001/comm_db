...
class LogMe extends Object {    
    ...
    private static function isNeedLog($level){
        $logLevels=getClassConsts("EnumLogLevel");
    ...
    public static function logPath($destination){
        if (empty(Gc::$log_config["logpath"])){
           Gc::$log_config["logpath"] = Gc::$nav_root_path."log".DIRECTORY_SEPARATOR;
        }
        if(empty($destination)){
            $destination=Gc::$log_config["logpath"].DIRECTORY_SEPARATOR.Gc::$appName.date('Y_m_d').self::SUFFIX_FILE_LOG;
        }  else {
            $destination=Gc::$log_config["logpath"].DIRECTORY_SEPARATOR.$destination.date('Y_m_d').self::SUFFIX_FILE_LOG;
        }       

        //检测日志文件大小，超过配置大小则备份日志文件重新生成
        if(is_file($destination) && (Gc::$log_config["log_file_size"] <= filesize($destination))){
            rename($destination,dirname($destination).DIRECTORY_SEPARATOR.basename($destination,self::SUFFIX_FILE_LOG).'-'.time().self::SUFFIX_FILE_LOG);
        }
    ...
    public static function record($message,$level=EnumLogLevel::ERR,$record=false) {
        if (Gc::$dev_debug_on){
            if($record || self::isNeedLog($level)) {        
                ...
                $level=getClassConstNameByValue("EnumLogLevel", $level);
